...
UI::Session::Session(std::string name_map): 
    ...
    {
        // In a real game, you might scan a directory for all tower JSON files
...
void UI::Session::mainSession() {
    ...
    Enemy ref{0.2, 5, 0.2, true};
    std::vector<Enemy*> el = {};
    Point spawningDirection = ((*path.begin())^(*(++path.begin())));
    while(running && !wants_to_die_) {
        auto now = clock::now();
        float logic_dt = std::chrono::duration<float>(now - lastTickTime).count();
        if(logic_dt < (1.0f / ticks_per_seconds_)) {
        ...
        // Update text sprites when values change
        ...
        auto timeSinceSpawn = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastSpawnTime).count();
        if (timeSinceSpawn >= 2000) { // toutes les 2 secondes
