...
Enemy::Enemy(float lp, float speed, float resistance, bool fly, std::string display_char, SDL_Color color, float size) :
    Entity{{0,0}, 0.0f, Enemy::createSprites(fly, display_char, color, size)}, lp_{lp}, max_lp_{lp}, speed_{speed}, resistance_{resistance}, fly_{fly}, display_char_{display_char}, path_{}, offset_{0.0f}, walk_timer_{static_cast<float>(rand()) / RAND_MAX * 0.5f} {}
...
Enemy::Enemy(Point position, float offset, const Enemy& ref, std::list<Point>::iterator start, std::list<Point>::iterator end) 
    ...
    path_end_ = end;
    offset_ = offset;
    walk_timer_ = static_cast<float>(rand()) / RAND_MAX * 0.5f; // Randomize walk sound timing
}
...
void Enemy::live(float deltaTime) {
    ...
    if (!fly_) {
        walk_timer_ += deltaTime;
        if (walk_timer_ >= 0.5f) { // Play every 0.5 second roughly
            MusicController::getInstance().playSoundSpatial("walk", position_.getX(), position_.getY(), 1.0f, 0.25f);
            walk_timer_ = 0.0f;
        }
    }
}
